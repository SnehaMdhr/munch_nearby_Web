...
export const getReviewsByRestaurant = async (restaurantId: string) => {
  try {
    ...
    throw new Error(
      err.response?.data?.message || err.message || "Fetching reviews failed",
    );
  }
};
  ...
  try {
    ...
    if (err?.response?.status === 404) {
      ...
      for (const path of fallbackPaths) {
        try {
          const fallbackResponse = await axiosInstance.post(path, reviewData);
    ...
    throw new Error(
      err.response?.data?.message || err.message || "Creating review failed",
    );
  }
};
...
export const updateReview = async (
  ...
  try {
    ...
    throw new Error(
      err.response?.data?.message || err.message || "Updating review failed",
    );
  }
};
...
export const deleteReview = async (reviewId: string) => {
  try {
    const response = await axiosInstance.delete(API.REVIEW.DELETE(reviewId));
    return response.data;
  } catch (err: Error | any) {
    if (err?.response?.status === 404) {
      try {
    ...
    throw new Error(
      err.response?.data?.message || err.message || "Deleting review failed",
    );
  }
};
...
export const getReviewsForOwner = async () => {
  try {
    const response = await axiosInstance.get(API.REVIEW.OWNER.GET_MY);
...
export const adminDeleteReview = async (reviewId: string) => {
  try {
    const response = await axiosInstance.delete(
      API.ADMIN.REVIEWS.DELETE(reviewId),
    );
    return response.data;
  } catch (err: Error | any) {
    throw new Error(
      err.response?.data?.message || "Admin deleting review failed",
    );
  }
};
